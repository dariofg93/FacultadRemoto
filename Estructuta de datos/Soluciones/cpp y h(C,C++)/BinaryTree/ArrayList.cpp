...
ArrayList crearArrayListDe(int initialCapacity) // O(1)
{
    ...
    xs->elems = new int[initialCapacity];
...
void agrandar(ArrayList& xs)
{
    ...
    int* nuevoArray   = new int[xs->maxSize]; // creo un array nuevo
...
void add(ArrayList& xs,  elemento x) // O(1) amortizado
{
...
// Prec.: length(xs) > 0
elemento remove(ArrayList& xs) // O(1)
{
    xs->lastIndex--;
}

// Precondicion: i < length(xs)
elemento getAt(ArrayList xs, int i) // O(1)
{
    return xs->elems[i];
}

// Precondicion: i < length(xs)
void setAt(ArrayList& xs, int i, elemento x) // O(1)
{
    xs->elems[i] = x;
}
...
int indexOf(ArrayList xs, elemento x) // O(n)
{
...
// en una posición
void addAt(ArrayList& xs, int i, elemento x) // O(n)
{
